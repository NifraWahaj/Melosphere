...
<body>
    ...
    <div class="d-flex customLinearGradient">
        ...
        <% if(session.getAttribute("LName") == null) {
            response.sendRedirect("ListenerSignUp.jsp");
        } else {
        ...
        <main class="container-md py-4" style="padding-left: 11em;">
        <h2 class="fs-1 fw-bold"><%=P.getName()  %></h2>
        <h3 class="fs-3 mb-3"><%=P.getDescription() %></h3>
       
       <hr>
         <form action="ListenerDeletePlaylist.jsp" class="dropdown-item L-Affiliate-Tagged">
            <input type="hidden" name="PName" id="PName" value="<%= P.getName() %>">
            <button type="submit" class="myBtn px-5">Delete Playlist</button>
           </form>
           <br><br>
        <form method="POST" action="ListenerAddSongsToPlaylistActual.jsp" class="dropdown-item L-Affiliate-Tagged">
            <input type="hidden" name="PName" id="PName" value="<%= P.getName() %>">
            <label for="songName">Song Name:</label>
            <input type="text" name="songName" id="songName">
            <label for="AName">Artist Name:</label>
            <input type="text" name="AName" id="AName">
            <button type="submit" class="myBtn px-5">Add Song To Playlist</button>
           </form>
		    ...
		    {%>
               <div class="row">
				   ...
				   <div class="col-2"> Genre   </div>
					   ...
					   <div class="col-1"> Streams </div>
			   </div>
			   <hr>
            
              <%List<Song> songList = P.getPlaylist();
 ...
 <div class="p-3 row text-whi te-50 shadow myBg-gray rounded my-3" >
	           ...
	           <button type="submit" class="myBtn" style="border: none; width: auto; background-color: transparent; color: #999999;"><%= song.getAName() %></button>
    				...
    				<li><a class="dropdown-item L-Affiliate-Tagged" href="#" onclick="copyToClipboard()">  Share  </a></li>
  					<li><form method="POST" action="ListenerAddToFav.jsp" class="dropdown-item L-Affiliate-Tagged">
  					<input type="hidden" name="AName" id="AName" value="<%= song.getAName() %>">
  					<input type="hidden" name="SName" id="SName" value="<%= song.getSName()  %>">
  					<button class="dropdown-item L-Affiliate-Tagged" type="submit">Add to Favorites</button>
						   </form></li>
