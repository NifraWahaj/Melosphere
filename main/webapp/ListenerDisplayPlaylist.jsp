...
<div class="d-flex customLinearGradient">
    ...
    <span class="fs-4 mt-4 myText-dark">Melosphere</span>
     ...
     <form class="nav-item mb-3" action="./search" method="get">
		...
		<h2 class="fs-1 fw-bold">Your Playlists</h2>
		<hr>
		
		
		<a href="ListenerCreatePlaylist.jsp"><button class="myBtn px-5" >Create Playlist</button></a>
		
		<% List<Playlist> p2 = (ArrayList<Playlist>)request.getAttribute("ListenerPlaylists");
           if(p2 != null && p2.size() > 0) {
           for(Playlist playlist2 : p2) {%>
        	
     
  		 
  		 	<section class="playlists mt-5">
				<hr>
                <div class="row">
                <div class=" m-3 bg-info p-1 fw-bold text-center rounded fs-4 py-3">
                        <box-icon name='deezer' type='logo' color='#ffffff' >
                        </box-icon>
                     
                     <div class=""><%=playlist2.getName()  %></div>
                     <div class=""><%=playlist2.getDescription()  %></div>
                     </div>
            
             <form action="ListenerShowPlaylistSongsDummy.jsp" class="linkStyling col-12">
			 <input type="hidden" name="PName" id="PName" value="<%= playlist2.getName() %>">
  			 <input type="hidden" name="PDes" id="PDes" value="<%= playlist2.getDescription() %>">
  		     <button type="submit" class="myBtn px-5" class="row">More</button>
			</form>
